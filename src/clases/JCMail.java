...
public class JCMail {
    ...
    String servidor,utiliza_conexion,autenticacion;
    String puerto;
    String[] tmp;
    ...
    public void SEND()
    {
        try {
            funcion.conectate();
            servidor=funcion.GetData("servidor", "select servidor from email");
            puerto= funcion.GetData("puerto", "select puerto from email");
            utiliza_conexion = funcion.GetData("utiliza_conexion_TLS", "select utiliza_conexion_TLS from email");
            autenticacion = funcion.GetData("utiliza_autenticacion", "select utiliza_autenticacion from email");
            
            boolean auxConex=false,auxAute=false;
            if(utiliza_conexion.equals("1")){
                auxConex=true;
            }
            ...
            props.put("mail.smtp.starttls.enable", auxConex);
            props.put("mail.smtp.auth", auxAute);
            ...
            if(Adjunto!=""){
                            ...
                            if(principal.status==true){
                             tmp =Adjunto.split("PDF");
                            }else{
                             tmp =Adjunto.split("CFD");
                            }
                            int j=0;
                            while( j < tmp.length){
    ...
    public void setPassword(String contraseña_emisor) {
